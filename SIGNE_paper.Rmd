...
#sp=droplevels(sp[which(sp$parcelle=="G"),]) #G
```
...
for (i in 1:length(set_name)) {
  ...
  writeMat(paste0("SIGNE_asca_",set_name[i],".mat"), x=spf$x, fact=fact, name_fact=name_fact)
}
```
...
# Only parcelle G
id=match("parcelle", colnames(fact))
writeMat("SIGNE_asca_parcG.mat", x=spf$x, fact=fact[,-id])


```


ASCA # Donne des resultats differents de ASCA Matlab (version )
```{r}
...
```{r}
ascaMat=readMat('SIGNE_asca_ev.mat')
#All
