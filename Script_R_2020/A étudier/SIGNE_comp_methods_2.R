...
brb4="C:/Users/Noémie/Desktop/SFE/Toutesdates/Globalmatrix/globalmatrix"
load(file=brb4)
...
#perok_finalPLSDAKNN =matrix(nrow =repet, ncol = ncmax)
...
#      rlda=lda(sc_cal[,1:ii], class_cal,tol=1.0e-5)
  ...
  #    pred[id_val,ii]=predict(rlda ,sc_val[,1:ii])$class
     ...
     # predPLSDAKNN[id_val,ii]=predict(fm, sc_val[,1:ii])$class
...
#    ts=lapply(as.list(pred), class, FUN = table)
  ...
  #  tsPLSDAKNN = lapply (as.list(predPLSDAKNN), class, FUN = table)
...
#    diags=lapply(ts, FUN = diag)
  ...
  #  diagsPLSDAKNN=lapply(tsPLASDAKNN, FUN = diag)
...
#    perok=100*unlist(lapply(diags, FUN = sum))/length(class)
  ...
  #  perokPLSDAKNN=100*unlist(lapply(diagsPLSDAKNN, FUN = sum))/length(class)
...
#  perok_final[j,]=perok
 ...
 # perok_finalPLSDAKNN[j,]=perokPLSDAKNN
...
#points(colMeans(perok_finalPLSDAKNN),col="green", pch=19, cex=1.5)
#legend(ncmax*2/3,15,legend=c("Maha on PLSDA scores", "KNN"),
 #      col=c("black","green"), lty=1, cex=0.8)
legend(ncmax*2/3,15,legend="Maha on PLSDA scores",
